...
public class BookRestController {
    ...
    @HystrixCommand(fallbackMethod = "getAllBooksDefault")
    ...
    public List<BookDtoWeb> getAllBooksDefault() {
        return List.of(new BookDtoWeb(1,"Book <>","", ""));
    }

    @HystrixCommand(fallbackMethod = "getBookDefault")
    @GetMapping("api/books/{id}")
    public Book getBook(@PathVariable("id") long id) {
        return bookService.findById(id).orElseThrow(AuthorNotFoundException::new);
    }

    public Author getBookDefault(@PathVariable("id") long id) {
        return new Author(id,"Book <>");
    }
